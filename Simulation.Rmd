...
alpha_boot <- function(data, indices) {
  result <- tryCatch({
    psych::alpha(data[indices, ], discrete = FALSE)$total$raw_alpha
  }, error = function(e) NA)
  return(result)
}
...
simulate_reliability_trial <- function(loading_type, n, n_items) {
  ...
  if (loading_type == "tau_equivalent") {
    ...
    loadings <- seq(0.4, 0.95, length.out = n_items)
  }
  ...
  alpha_obs <- psych::alpha(Y, discrete = FALSE)$total$raw_alpha
  bootstrapped_alpha <- boot(data = Y, statistic = alpha_boot, R = 200)
  alpha_ci <- boot.ci(bootstrapped_alpha, type = "perc")$percent[4:5]


  # Observed McDonalds Omega (with CI)
  omega_obs <- lavaan_omega(Y)
  bootstrapped_omega <- boot(data = Y, statistic = omega_boot, R = 200)
